...
function parseDescription(desc: any): string {
  ...
  if (typeof desc === "object") {
    if (Array.isArray(desc.blocks))
      return desc.blocks.map((b: any) => b.text || "").join("\n\n");
...
function getCategoryDisplayName(c: any): string {
  ...
  return (
    ...
    c.categoryName ??
    c.category?.name ??
    (c.category?.catgoryId ? String(c.category?.name ?? c.category?.catgoryId) : JSON.stringify(c))
  );
}

/** Quiz preview */
const QuizPreview: React.FC<{ quizId: number }> = ({ quizId }) => {
  const { data: quiz, isLoading } = useGetQuizDetailQuery(quizId, {
    skip: !quizId || quizId === 1,
  });
                ...
                <List.Item key={q.questionId ?? idx}>
                  ...
                  <div style={{ marginTop: 8 }}>
                    {answers.length ? (
                        ...
                        renderItem={(ans, aidx) => (
                          <List.Item>
                            ...
                            >
                              ...
                              <Text
                                style={
                                  aidx === correctIndex ? { fontWeight: 700 } : undefined
                                }
                              >
                      ...
                      <Text type="secondary">
                        Không có đáp án (raw: {String(raw)})
                      </Text>
                    )}
...
export default function ReviewCourseTeacher(): JSX.Element {
  ...
  const {
    ...
    isError,
    refetch,
  } = useGetCourseIDQuery(id ?? "");
  const [requestApproveCourse, { isLoading: isRequesting }] =
    useRequestApproveCourseMutation();

  const [publishCourse, { isLoading: isPublishing }] =
    usePublishCourseMutation();

  const [isActiveToggle, setIsActiveToggle] = useState<boolean>(() =>
    Boolean(course?.courseIsActive)
  );
  useEffect(() => {
    if (course) setIsActiveToggle(Boolean(course.courseIsActive));
  }, [course?.courseIsActive]);

  // ⭐ NEW: Load approve history
  const courseIdNumber = Number(id);
  const {
    data: approveHistory = [],
    refetch: refetchApprove,
  } = useGetApproveHistoryQuery(courseIdNumber, { skip: !courseIdNumber });
  const latestApprove = approveHistory[0];
  const hasPending = latestApprove?.approveStatus === "Pending";
  ...
  if (!course) {
    return (
      <Card style={{ margin: 24 }}>
        <Text type="danger">
          {isError ? "Lỗi khi tải khóa học" : "Không tìm thấy khóa học"}
        </Text>
      </Card>
    );
  }
      ...
      <Card bordered={false} className="mb-6">
        <Row gutter={16} align="middle">
          ...
          <Col xs={24} lg={18}>
            ...
            >
              {(course.courseCategories ?? course.courseCategories ?? []).length >
                0 ? (
                (course.courseCategories ?? course.courseCategories ?? []).map(
                  (c: any, idx: number) => (
                    <Tag
                      key={c.categoryId ?? c.catgoryId ?? idx}
                      color="blue"
                    >
                      {getCategoryDisplayName(c.category ?? c)}
                    </Tag>
                  )
                )
              ) : (
            ...
            >
              {typeof course.price === "number" ? (
                <div>
                  ...
                  <Text>{course.price.toLocaleString()} ₫</Text>
                </div>
              ) : null}

              {course.salesCampaign != null ? (
                <div>
                  <Text strong>Giảm:</Text> <Text>{course.salesCampaign}</Text>
                </div>
              ) : null}

              <div>
                <Text strong>Trạng thái hoạt động:</Text>{" "}
                <Text>{course.courseIsActive ? "Hoạt động" : "Tạm ẩn"}</Text>
              </div>
            </div>
          </Col>
        </Row>
      ...
      {Array.isArray(course.sections) && course.sections.length > 0 ? (
        course.sections.map((section: any, sIdx: number) => (
          <Card
            ...
            title={`Chương ${sIdx + 1}: ${section.title || "(Chưa đặt tên)"}`}
            className="mb-4"
            bordered
          >
            <Collapse
                ...
                section.steps.map((step: any, idx: number) => {
                  ...
                  const panelHeader = (
                    ...
                    >
                      ...
                      >
                        <div>
                          <Text strong>Bài {idx + 1}:</Text>{" "}
                          <Text>{step.title || "(Chưa đặt tên bài)"}</Text>
                        </div>
                      ...
                      <div style={{ color: "#6b7280", fontSize: 13 }}>
                        <span style={{ marginRight: 12 }}>
                          Vị trí: {step.position ?? idx + 1}
                        </span>
                        {step.duration ? (
                          <span>Thời lượng: {step.duration}s</span>
                        ) : null}
                      </div>
                    </div>
                  );
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ marginBottom: 8 }}>
                            ...
                            {stepDesc ? (
                              ...
                              <div>
                                <Text type="secondary">Không có mô tả bài</Text>
                              </div>
                            )}
                          </div>
        ...
        <Card bordered className="mb-4">
          <Text type="secondary">Khóa học chưa có chương nào.</Text>
        </Card>
      )}

      <Divider />

      {/* ⭐ NEW: Lịch sử duyệt khóa học */}
      <Card title="Lịch sử duyệt khóa học" bordered className="mb-6">
        {approveHistory.length === 0 ? (
          <Text type="secondary">Chưa có lịch sử duyệt.</Text>
        ) : (
          <List
            itemLayout="vertical"
            dataSource={approveHistory}
            renderItem={(item: any) => (
              <List.Item>
                <Space direction="vertical" style={{ width: "100%" }}>
                  <div>
                  ...
                  {item.comment ? (
                    <div>
                      <Text strong>Ghi chú:</Text> <Text>{item.comment}</Text>
                    </div>
                  ) : null}
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <Switch
              ...
              disabled
              onChange={(v) => setIsActiveToggle(v)}
            />
          </div>
          ...
          <Button
            ...
            onClick={async () => {
              if (hasPending) {
                message.warning("Bạn đã gửi yêu cầu trước đó và đang chờ Admin duyệt.");
                return;
              }

              try {
                await requestApproveCourse(Number(course.courseId)).unwrap();
                await publishCourse({
                  courseId: Number(course.courseId),
                  isActive: true,
                } as any).unwrap();
                message.success("Đã gửi yêu cầu duyệt khóa học đến Admin");

                refetch?.();
                refetchApprove?.();
              } catch {
                message.error("Gửi yêu cầu duyệt thất bại");
              }
            }}
          >
            {hasPending ? "Đang chờ duyệt..." : "Gửi tới Admin để xuất bản"}
          </Button>
