...
namespace LMSystem.API.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        public async Task<IActionResult> SignUpTeacher(SignUpModel signUpModel)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    var result = await _accountService.SignUpAccountTeacherAsync(signUpModel);
        ...
        public async Task<IActionResult> ConfirmCreateSchoolManagerAccount(int accountId, AccountStatusEnum accountStatus)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    var result = await _accountService.ConfirmCreateSchoolManagerAccount(accountId, accountStatus);
                    var  account = await _accountService.GetAccountById(accountId.ToString());
                    if (result.Status.Equals("Success"))
                    {
                        var messageRequest = new EmailRequest
                        {
