...
namespace LMSystem.Repository.Repositories
{
    public class AccountRepository : IAccountRepository
    {
        ...
        public async Task<ResponeModel> SignUpAccountTeacherAsync(SignUpModel model)
        {
            try
            {
                ...
                if (exsistAccount == null)
                {
                    ...
                    if (result.Succeeded)
                    {
                        if (!await roleManager.RoleExistsAsync(RoleModel.Teacher.ToString()))
                        {
                            await roleManager.CreateAsync(new IdentityRole(RoleModel.Teacher.ToString()));
                        }
                        if (await roleManager.RoleExistsAsync(RoleModel.Teacher.ToString()))
                        {
                            await userManager.AddToRoleAsync(user, RoleModel.Teacher.ToString());
                        }
                        //var token = userManager.GenerateEmailConfirmationTokenAsync(user);
                        //token = WebEncoders.Base64UrlEncode(Encoding.UTF8.GetBytes(token));
